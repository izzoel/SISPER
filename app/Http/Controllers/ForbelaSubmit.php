...
class ForbelaSubmit extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('pembayaran')) {
            ...
            // Panggil fungsi uploadFileToDrive untuk meng-upload file
            $uploadResult = $this->googleService->uploadFileToDrive($file, $filename, $folderId, true); // true untuk membuat file publik

            // Cek apakah upload berhasil
            if ($uploadResult && isset($uploadResult['url'])) {
                $filenameSaved = $uploadResult['url'];
            } else {
                $filenameSaved = null;
            }
        } else {
